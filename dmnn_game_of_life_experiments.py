...
for i in [5]:#range(len(net)):
    print(i)
    results[i] = dmnn.train_dmnn(x,y,net[i],dmnn.IoU,xval = xval,yval = yval,sample = True,neighbors = 256,epochs = 100000,batches = 1,notebook = True,epoch_print= 100,epoch_store = 10)
    ...
    tmp_table.to_csv('dmnn_gol_2bacthes_256_IoU_' + str(i) + '.csv')
    jnp.save("params_5_256_IoU.npy",results[i]['best_par'])
    del tmp_table
