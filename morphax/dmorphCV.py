...
def erosion(f,k):
    ...
    eb = cv.erode(img,k,iterations = 1)[:,l:-l,l:-l]
    return eb
...
def dilation(f,k):
    ...
    l = math.floor(k.shape[0]/2)
    f = np.pad(f, [(0,0), (l,l), (l,l)], mode='constant')
    db = cv.dilate(img,k,iterations = 1)[:,l:-l,l:-l]
    return db
...
def opening(f,k):
    ...
    l = math.floor(k.shape[0]/2)
    f = np.pad(f, [(0,0), (l,l), (l,l)], mode='constant')
    op = cv.morphologyEx(img, cv.MORPH_OPEN, k)[:,l:-l,l:-l]
    return op
...
def closing(f,k):
    ...
    l = math.floor(k.shape[0]/2)
    f = np.pad(f, [(0,0), (l,l), (l,l)], mode='constant')
    cl = cv.morphologyEx(img, cv.MORPH_CLOSE, k)[:,l:-l,l:-l]
    return cl
...
def sup(f):
    ...
    """
    fs = np.apply_along_axis(np.max,0,f)
...
def inf(f):
    ...
    """
    fi = np.apply_along_axis(jnp.min,0,f)
