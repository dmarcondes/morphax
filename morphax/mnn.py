...
def apply_morph_layer(x,type,params,index_x,forward_wop = None,alpha = 5,d = None,m = 255):
    ...
    if type != "wop" and type[0] != 'G':
        ...
        for i in range(params.shape[0]):
            if type == 'Gerosion':
                fx_tmp = oper(x,index_x,jnp.append(jnp.zeros((1,params.shape[2],params.shape[3])),params[i,:,:,:],0)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            elif type == 'Gdilation':
                fx_tmp = oper(x,index_x,jnp.append(params[i,:,:,:],jnp.zeros((1,params.shape[2],params.shape[3])) + 1.0,0)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            elif type == 'Ganti-erosion':
                fx_tmp = oper(x,index_x,jnp.append(1.0 - params[i,:,:,:],jnp.zeros((1,params.shape[2],params.shape[3])),0)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            elif type == 'Ganti-dilation':
                fx_tmp = oper(x,index_x,jnp.append(1.0 + jnp.zeros((1,params.shape[2],params.shape[3])),1.0 - params[i,:,:,:],0)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            elif type in ['Gopening','Gclosing','Gasf']:
                fx_tmp = oper(x,index_x,jnp.append(jnp.append(jnp.zeros((1,params.shape[2],params.shape[3])),params[i,:-1,:,:],0),jnp.zeros((1,params.shape[2],params.shape[3])) + 1.0,0)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            elif type == 'Gsupgen':
                b1 = jnp.append(jnp.zeros((1,params.shape[3],params.shape[4])),params[i,0,:,:,:],0)
                b2 = jnp.append(1.0 + jnp.zeros((1,params.shape[3],params.shape[4])),1.0 - params[i,1,:,:,:],0)
                fx_tmp = oper(x,index_x,jnp.minimum(b1,b2),jnp.maximum(b1,b2)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            elif type == 'Ginfgen':
                b1 = jnp.append(1.0 - params[i,0,:,:],jnp.zeros((1,params.shape[3],params.shape[4])),0)
                b2 = jnp.append(params[i,1,:,:],jnp.zeros((1,params.shape[3],params.shape[4])) + 1.0,0)
                fx_tmp = oper(x,index_x,jnp.minimum(b1,b2),jnp.maximum(b1,b2)).reshape((1,x.shape[0],x.shape[1],x.shape[2]))
            if fx is None:
                fx = fx_tmp
            else:
...
def cmnn_fcnn(type,width,width_str,size,shape_x,width_wop = None,activation = jax.nn.relu,sample = True,mean = 0,sd = 0,key = 0,loss = MSE_SA,sa = True,c = 100,q = 2,epochs = 5000,lr = 0.001,b1 = 0.9,b2 = 0.999,eps = 1e-08,eps_root = 0.0,notebook = False,epochs_print = 500,alpha = 5,m = 255):
    ...
    def forward_general(w,params,d):
        ...
        return jax.nn.sigmoid(jax.vmap(lambda x: x + ax)(bx))
