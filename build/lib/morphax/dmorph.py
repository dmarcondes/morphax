...
def erosion(f,k):
    ...
    eb = binary_erosion(f,k.reshape((1,k.shape[0],k.shape[1])))[:,l:-l,l:-l]
...
def dilation(f,k):
    ...
    l = math.floor(k.shape[0]/2)
    f = np.pad(f, [(0,0), (l,l), (l,l)], mode='constant')
    db = binary_dilation(f,k.reshape((1,k.shape[0],k.shape[1])))[:,l:-l,l:-l]
            ...
            if sample:
                if type[i] == 'supgen' or type[i] == 'infgen':
                    ...
                    for j in range(width[i]):
                        s = rng.choice(2,p = np.array([1 - p1,p1]),size = (1,1,size[i],size[i]))
                        #k = k + 1
                        s = np.maximum(ll,s)
                        if j == 0:
                            p = np.append(s,ul,1)
...
def step_slda(params,x,y,forward,lf,type,width,size,neighbors = 8,key = 0):
    ...
    layers = rng.choice(len(prob),size = (neighbors,),p = prob)
    hood = None
    ...
    for i in range(neighbors):
        ...
        tmp_prob = tmp_prob/np.sum(tmp_prob)
        node = rng.choice(max_width,size = (1,),p = tmp_prob)
        #Sample row and collumn
        tmp_prob = count[node,:,:].reshape((max_size ** 2)).astype(np.float32)
        tmp_prob = tmp_prob/np.sum(tmp_prob)
        tmp_prob = np.append(tmp_prob[:-1],np.array(1 - np.sum(tmp_prob[:-1])))
        tmp_prob = tmp_prob/np.sum(tmp_prob)
        tmp_random = rng.choice(max_size ** 2,size = (1,),p = tmp_prob)
...
def train_dmnn_slda(x,y,net,loss,xval = None,yval = None,stack = False,neighbors = 8,epochs = 1,batches = 1,K = 255,notebook = False,epoch_print = 100,epoch_store = 1,key = 0,store_jumps = False):
    """
    ...
    with alive_bar(epochs) as bar:
        ...
        for e in range(epochs):
            ...
            if batches > 1:
                if stack:
                    per = rng.permutation(np.arange(x.shape[1]))
                    x = x[:,per,:,:]
                else:
                    per = rng.permutation(np.arange(x.shape[0]))
                    x = x[per,:,:]
                y = y[per,:,:]
            for b in range(batches):
                if batches > 1:
                    ...
                    #Search neighbors
                    params,jumps,train_loss = update(params,xb,yb,round(1000000*rng.uniform(size = 1)[0]),jumps)
                else:
                    #Search neighbors
                    params,jumps,train_loss = update(params,x,y,round(1000000*rng.uniform(size = 1)[0]),jumps)
            ...
            bar()
    return {'best_par': best_par,'jumps': jumps,'trace_epoch': trace_epoch,'trace_time': trace_time,'trace_loss': trace_loss,'trace_val_loss': trace_val_loss,'epochs': epochs,'oper': oper,'forward': forward}
